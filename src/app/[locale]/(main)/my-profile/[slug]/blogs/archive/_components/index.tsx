...
import { useProfileBlogPostsArchivesQuery } from "../../_hooks/use-profile-blog-posts-archives-query"

export const BlogArchive = () => {
    const t = useTranslations()
    const params = useSearch()
    const pageSize = 12
    const { data, blogPostsArchive } = useProfileBlogPostsArchivesQuery({
        params: {
            ...params,
            page_size: pageSize,
        },
    ...
    return (
        <>
            {blogPostsArchive.length > 0 ?
                <div className="grid gap-3 grid-cols-[repeat(auto-fill,minmax(17.6rem,1fr))]">
                    {blogPostsArchive.map((item) => (
                        <BlogsCard key={item.slug} fromArchive item={item} />
                    ))}
                </div>
            :   <Empty
                    emptyIcon={<IconBlogEmpty />}
                    title={t("noArchivedBlogs")}
