...
export default function LeftSide() {
    ...
    const { totalSelected } = useMemo(() => {
        ...
        const requestTypeCount = getArray(params.request_type).length
        const statusCount = getArray(params.status).length
        const timeCount = getArray(params.created_at).length
        return {
            countryCount,
            totalSelected:
                requestTypeCount + statusCount + timeCount + countryCount,
        }
    }, [
        params.request_type,
        params.status,
    ...
    function handleAiClick() {
        openAiModal("outreach_hub_filter")
    }
            ...
            <div className="flex items-center py-0 justify-between md:sticky md:top-24">
                <hgroup className="flex flex-col gap-4 w-full">
                    <main
                        ...
                        className="cursor-pointer border-gradient w-fit self-start md:w-full md:self-stretch p-2.5"
                    >
                        ...
                        {/* <div className="hidden md:flex flex-col gap-2 mt-6">
                            ...
                            <p className="text-sm">
                                Hey, I want to publish a new outreach post on
                                the Outreach Hub ...
                            </p>
                        </div> */}
                    </main>
